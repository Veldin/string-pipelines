...
class Decrementtest {
    ...
    static Stream<Arguments> cases() {
        return Stream.of(
                Arguments.of("bcd", "abc"),
                Arguments.of("BCD", "ABC"),
                Arguments.of("123", "012"),
    ...
    @ParameterizedTest
    @MethodSource("cases")
    void decrementtest(String input, String expected) {
        CodePointBuffer buffer = new CodePointBuffer(input.codePoints().toArray());

        CodePointUtils.decrement(buffer);
    ...
    void pipelineStepTest() {
        ...
        AbstractPipeline pipeline =
                new OperationsPipelineBuilder()
                        .pipe(ECodePointOperation.DECREMENT)
                        .build();
